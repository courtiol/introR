...
Projects can be directly open from your file explorer.
...
<br>For example, the function `sum()` takes a numeric vector and returns a single value.
...
::: {.fragment}
::: callout-note
`c()` is itself a function creating the numeric vector.
<br>We'll discuss more what a **vector** is soon.
:::
:::
...
Using the syntax `package::function()` makes it explicit which package to use.
...
<br>
$\geq$ 1 values of the **same class**.
...
<br>
$\geq$ 1 values of the [**same class**]{style="color:#ff2c2d;"}.
...
# NULL: Empty
# NA: Missing data (can be any class)
...
<br>
$\geq$ 1 value(s) of the **same class** with **two dimensions**.
...
<br>
$\geq$ 1 value(s) of the [**same class**]{style="color:#ff2c2d;"} with **two dimensions**.
...
<br>
$\geq$ 1 value(s) of the **same class** with [**two dimensions**]{style="color:#ff2c2d;"}.
...
::: callout-note
Remember, we have here two dimensions.
...
Each item can be a [**different class**]{style="color:#ff2c2d;"}.
...
```{r}
#| echo: true
## Use lists to store objects of different classes together
...
```{r}
#| echo: true
## Use `[[]]` to extract items from the list
## Return list item 1
my_list[[1]]
```
:::
...
<br>
When list-elements are named, they can be extracted using either `[[]]` or `$`.

```{r}
#| echo: true
## Use `[[]]` to extract the items inside the list
...
::: {.fragment}
::: callout-note
If the name is defined via an object, only `[[]]` will do.
:::

```{r}
#| echo: true
## Define the element name indirectly
what_I_want <- "second"
my_named_list[[what_I_want]]
```
:::
...
::: callout-note
Many advanced functions store their output as a list object.
...
```{r}
#| echo: true
# Internally, there is a lot more information!
str(model)
```
...
- Each item in the list (i.e. column) is a vector ([**same class**]{style="color:#ff2c2d;"}).
